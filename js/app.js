...
let previousView   = null;   // view ID before navigating to overview/actions
...
function showView(name){
  // Track the previous view so overview/actions can return cleanly
  const cur = document.querySelector('.view.active');
  const curId = cur ? cur.id.replace('view-','') : null;
  if(curId && curId !== 'overview' && curId !== 'actions'){
    previousView = curId;
  }
...
function checkFormAvailability(formCode, scopeKey, submittedKey){
  ...
  if(missing.length){
    ...
    return {
      available: true,          // Forms are always accessible — no hard blocking
      locked: false,
      warning: true,
      warnReason: 'Sequence note: ' + names.join(', ') + ' not yet submitted',
    };
  }
  return { available: true, locked: false, warning: false, warnReason: '' };
}
...
function renderFormsView(){
  ...
  cfg.forms.forEach(f => {
    ...
    if(avail.warning){
      // Workflow sequence warning — but form is still accessible
      let warnBadge = `<div class="card-status-row"><span class="status-chip status-seq-warn">⚠ Sequence</span>`;
      if(existing){
        const ls = existing.lifecycleStatus;
        const ins = existing.inspectionStatus;
        warnBadge += `<span class="status-chip status-${ls.toLowerCase()}">${ls}</span>`;
        if(ins) warnBadge += `<span class="status-chip status-${ins}">${ins}</span>`;
        warnBadge += `<span class="card-rev-badge">REV${String(existing.revision||0).padStart(2,'0')}</span>`;
        if(ls === 'Submitted') cardClass += ' card-submitted';
        else if(ls === 'Draft') cardClass += ' card-draft';
        else if(ls === 'Closed') cardClass += ' card-closed';
      }
      warnBadge += `</div><div class="seq-warn-reason">${avail.warnReason}</div>`;
      statusBadge = warnBadge;
    } else if(existing){
      const ls = existing.lifecycleStatus;
      const ins = existing.inspectionStatus;
...
function openFormOrRegister(formCode, scopeKey){
  ...
  // Sequence check — informational only, does not block
  const avail = checkFormAvailability(formCode, scopeKey, submittedKey);
  if(avail.warning){
    showToast(avail.warnReason, 'warn');
    // Continue — form is still accessible
  }
    ...
    (formDef.sections || []).forEach(section => {
      (section.fields || []).forEach(field => {
        if(TABLE_DEFS[field.type] && prefill[field.id] && Array.isArray(prefill[field.id])){
          ...
          // Restore attachment previews — normalise legacy base64 strings
          const hidden = document.getElementById('f_' + field.id);
          if(hidden){
            const normalised = prefill[field.id].map((a,i) =>
              typeof a === 'string' ? {name:`Photo ${i+1}`, url:a, source:'temp_browser', addedAt:''} : a
            );
            hidden.value = JSON.stringify(normalised);
            if(!readOnly) refreshAttachPreview(field.id, normalised, hidden);
          }
        }
      });
    });
...
function renderField(field, prefill, readOnly){
  ...
  // Record structure: attachments[] = [{name, url, source, addedAt, tempBlob?}]
  // source: 'temp_browser' | 'sharepoint' | 'onedrive' | 'url'
  if(field.type === 'photo'){
    const attachments = (prefill && prefill[field.id]) ? prefill[field.id] : [];
    // Normalise: older format was array of base64 strings
    const normAttachments = attachments.map((a,i) =>
      typeof a === 'string' ? {name:`Photo ${i+1}`, url:a, source:'temp_browser', addedAt:''} : a
    );

    if(readOnly){
      const attHtml = normAttachments.length
        ? normAttachments.map(a => `
          <div class="attach-item">
            ${a.source === 'temp_browser'
              ? `<img class="attach-thumb" src="${a.url}" alt="${a.name}">`
              : `<div class="attach-link-icon">📎</div>`}
            <div class="attach-meta">
              <span class="attach-name">${a.name}</span>
              ${a.url && a.source !== 'temp_browser' ? `<a class="attach-link" href="${a.url}" target="_blank" rel="noopener">Open ↗</a>` : ''}
              <span class="attach-source">${a.source === 'temp_browser' ? 'Temp (browser)' : a.source || ''}</span>
            </div>
          </div>`).join('')
        : `<div class="photo-empty">No attachments.</div>`;
      return `<div class="f-group f-full"><label>${field.label}${req}</label>${hint}<div class="attach-list">${attHtml}</div></div>`;
    }

    // Editable — support temp browser upload + URL reference entry
    const previewId = 'attach_preview_' + field.id;
    return `<div class="f-group f-full photo-upload-group">
      <label>${field.label}${req}</label>
      ${hint}
      <div class="attach-model-note">
        <strong>Attachment storage:</strong> Photos attached here are stored temporarily in your browser.
        When SharePoint/cloud integration is configured, attachments will link to the project file store instead.
      </div>
      <div class="photo-controls">
        <label class="btn-photo-upload" for="photo_input_${field.id}">📷 Add Photo (temp)</label>
        ...
        <button type="button" class="btn-attach-ref" onclick="addAttachmentRef('${field.id}')">🔗 Add Reference URL</button>
      </div>
      <div class="attach-list" id="${previewId}"></div>
...
// Photo upload handler — converts to base64 blob (temp) using reference model
function handlePhotoUpload(fieldId, input){
  const hidden = document.getElementById('f_' + fieldId);
  let attachments = [];
  try { attachments = hidden.value ? JSON.parse(hidden.value) : []; } catch(e){ attachments = []; }

  const files = Array.from(input.files);
  if(!files.length) return;

  if(attachments.length + files.length > 8){
    showToast('Warning: many attachments may hit browser storage limits. Consider SharePoint links instead.', 'warn');
  }

  let loaded = 0;
  files.forEach(file => {
    if(file.size > 2 * 1024 * 1024){
      showToast(`"${file.name}" is over 2 MB. Use a reference URL for large files.`, 'warn');
      loaded++;
      if(loaded === files.length) refreshAttachPreview(fieldId, attachments, hidden);
      return;
    }
    ...
    reader.onload = e => {
      attachments.push({
        name: file.name,
        url:  e.target.result,   // temp base64 blob
        source: 'temp_browser',
      ...
      if(loaded === files.length){
        ...
        refreshAttachPreview(fieldId, attachments, hidden);
      }
    };
...
// Add a URL reference (SharePoint / OneDrive / any link)
function addAttachmentRef(fieldId){
  const url = prompt('Enter the attachment URL (e.g. SharePoint link):');
  if(!url || !url.trim()) return;
  const name = prompt('Label for this attachment:', url.split('/').pop() || 'Attachment') || 'Attachment';
  const hidden = document.getElementById('f_' + fieldId);
  let attachments = [];
  try { attachments = hidden.value ? JSON.parse(hidden.value) : []; } catch(e){ attachments = []; }
  attachments.push({
...
function refreshAttachPreview(fieldId, attachments, hiddenInput){
  const preview = document.getElementById('attach_preview_' + fieldId);
  if(!preview) return;
  ...
  preview.innerHTML = attachments.map((a,i) => `
    <div class="attach-item">
      ...
      <div class="attach-meta">
        <span class="attach-name">${a.name}</span>
        ${a.source !== 'temp_browser' && a.url
          ? `<a class="attach-link" href="${a.url}" target="_blank" rel="noopener">Open ↗</a>` : ''}
        <span class="attach-source">${a.source === 'temp_browser' ? '⚠ Temp (browser only)' : a.source}</span>
      </div>
      <button type="button" class="photo-remove" onclick="removeAttachment('${fieldId}',${i})" title="Remove">✕</button>
    </div>`).join('');
}

function removeAttachment(fieldId, idx){
  const hidden = document.getElementById('f_' + fieldId);
  let attachments = [];
  try { attachments = hidden.value ? JSON.parse(hidden.value) : []; } catch(e){ attachments = []; }
...
function submitEntry(lifecycle){
  ...
  allFields.forEach(field => {
    if(TABLE_DEFS[field.type]){
      ...
      // Parse stored attachment reference array from hidden field
...
// ═══════════════════════════════════════════════════════
// OPEN RECORD FROM OVERVIEW (restores navigation context)
// ═══════════════════════════════════════════════════════
function openRecordFromOverview(recordId){
  const record = forms.find(f => f.id === recordId);
  if(!record){ showToast('Record not found.', 'err'); return; }

  // Restore full navigation context from record
  currentProject = projects.find(p => p.id === record.projectId) || currentProject;
  currentBlock   = projectBlocks.find(b => b.id === record.blockId);
  currentLevel   = projectLevels.find(l => l.id === record.levelId);
  currentArea    = projectAreas.find(a => a.id === record.areaId);
  currentScope   = record.scopeKey;
  currentRegisterForm = { formCode: record.formCode, scopeKey: record.scopeKey };
  ...
  pendingRevision = null;
  currentOutcome = record.inspectionStatus || null;
  renderFormEntry(record.data, record.lifecycleStatus !== 'Draft', record.lifecycleStatus);
  showView('entry');
}
...
function renderOverview(){
  ...
  const backLabel = previousView ? '← Back' : '← Back to Project';
  const el = document.getElementById('overview-stats');
  if(el) el.innerHTML = `
    <div class="page-nav-row overview-back-row">
      <button class="btn btn-small btn-back" onclick="goBackFromOverview()">${backLabel}</button>
    </div>
    ...
    <div class="recent-list">
      ...
      ${projectForms.length === 0 ? '<p class="empty-msg">No submissions yet.</p>' : ''}
      ${projectForms.slice(-30).reverse().map(f => `
        <div class="recent-item recent-item-link" onclick="openRecordFromOverview('${f.id}')" title="Open ${f.formRef}">
          <span class="recent-ref">${f.formRef}</span>
          <span class="recent-title">${f.formTitle}</span>
          <span class="recent-by">${f.submittedByName || '—'}</span>
          <span class="recent-date">${f.submittedAt ? f.submittedAt.slice(0,10) : (f.savedAt ? f.savedAt.slice(0,10) : '')}</span>
          <span class="recent-lifecycle status-chip status-${(f.lifecycleStatus||'').toLowerCase()}">${f.lifecycleStatus || ''}</span>
          ${f.inspectionStatus ? `<span class="status-chip status-${f.inspectionStatus}">${f.inspectionStatus}</span>` : ''}
          <span class="recent-open-arrow">›</span>
        </div>`).join('')}
    </div>`;
...
function renderActionCards(){
  ...
  const backBtn = `<div class="page-nav-row actions-back-row">
    <button class="btn btn-small btn-back" onclick="goBackFromActions()">← Back</button>
  </div>`;

  if(!projectActions.length){
    list.innerHTML = backBtn + '<p class="empty-msg">No actions recorded yet.</p>';
    return;
  }
  list.innerHTML = backBtn + projectActions.slice(-50).reverse().map(a => {
    const clickable = a.formId
      ? `class="action-item action-item-link" onclick="openRecordFromOverview('${a.formId}')"`
      : `class="action-item"`;
    return `<div ${clickable}>
      ...
      ${a.formId ? `<span class="action-open">›</span>` : ''}
    </div>`;
  }).join('');
}
...
function goBackFromOverview(){
  // Return to the view the user came from, or fall back to project
  if(previousView && previousView !== 'overview' && previousView !== 'actions'){
    const vEl = document.getElementById('view-' + previousView);
    if(vEl){
      // Re-render the previous view so it's populated
      if(previousView === 'forms')         renderFormsView();
      else if(previousView === 'scope')    renderScopeCards();
      else if(previousView === 'areas')    renderAreaCards();
      else if(previousView === 'levels')   renderLevelCards();
      else if(previousView === 'blocks')   renderBlockCards();
      else if(previousView === 'projects') renderProjectCards();
      showView(previousView);
      return;
    }
  }
  // Fallback: go to blocks if project set, else projects
  if(currentProject && currentBlock){ renderBlockCards(); showView('blocks'); }
  else if(currentProject){ renderProjectCards(); showView('projects'); }
  else showView('projects');
}

function goBackFromActions(){
  // Return to overview if project set, otherwise wherever we came from
  if(currentProject){
    renderOverview();
  } else if(previousView && previousView !== 'actions'){
    showView(previousView);
  } else {
    showView('projects');
  }
...
function goBack(){
  ...
  if(activeId === 'view-overview'){
    goBackFromOverview();
  } else if(activeId === 'view-actions'){
    goBackFromActions();
  } else if(activeId === 'view-entry'){
